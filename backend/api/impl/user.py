...
@require_auth
@post("retweet")
@contract(Schema({'tweet_id': Use(int)}))
def retweet(request):
    tweet_id = request.payload['tweet_id']
    tweet = service.tweet.get(tweet_id)

    if tweet is None:
        return client_error('INVALID_PARAM', "No such tweet.")

    
    if request.user.has_retweeted(tweet):
        return client_error('ALREADY_RETWEETED', "You've already retweeted this tweet.")

    
    retweet = service.retweet.create(request.user, tweet)

    return success({'retweet_id': retweet.id})